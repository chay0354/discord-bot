...
def resolve_symbol(symbol: str) -> Optional[Dict[str, Any]]:
    """Authoritative exact-symbol lookup via Yahoo's quote endpoint (no fuzzy search).

    Returns {symbol, shortName, exchange, marketCap, category, exchange_ok} or
    None when the symbol does not resolve to a real equity quote.
    """
    normalized = (symbol or "").upper().strip().lstrip("$")
    if not normalized:
        return None
    try:
        quotes = _fetch_quote_batch([normalized])
    except Exception as e:  # noqa: BLE001
        print("[yahoo_client] resolve_symbol error:", repr(e))
        return None
    info = quotes.get(normalized)
    if not info:
        return None
    qtype = str(info.get("quoteType") or "").upper()
    if qtype and qtype not in {"EQUITY", "S"}:
        return None
    mcap = info.get("marketCap")
    return {
        "symbol": normalized,
        ...
        "marketCap": mcap,
        "category": _classify_by_cap(mcap),
        "exchange_ok": _is_allowed_exchange(info),
        "source": "yahoo",
    }
...
def validate_symbol_for_category(symbol: str, category: str) -> Optional[Dict[str, Any]]:
    """
    Return a normalized row when symbol is a common stock on NASDAQ/NYSE
    and currently belongs to the expected market-cap category.
    """
    normalized = (symbol or "").upper().strip().lstrip("$")
    if not normalized:
        return None
    row = resolve_symbol(normalized)
    if row and row.get("exchange_ok") and row.get("category") == category:
        return row
    try:
