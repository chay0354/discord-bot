...
from config import CHANNEL_BLUE_LIVE, CHANNEL_BLUE_TICKER, CHANNEL_BLUE_VOTE


RENAMES = {
    "large-chip-ticker": CHANNEL_BLUE_TICKER,
    "large-chip": CHANNEL_BLUE_VOTE,
    "large-chip-live": CHANNEL_BLUE_LIVE,
}
    ...
    async def on_ready(self) -> None:
        for guild in self.guilds:
            ...
            for old_name, new_name in RENAMES.items():
                ...
                if old_channel:
                    await old_channel.edit(name=new_name, reason="Rename large chip channels to large cap")
                    print(f"Renamed #{old_name} -> #{new_name}", flush=True)
                elif existing_new:
                    print(f"Already renamed: #{new_name}", flush=True)
                else:
                    print(f"Missing channel: #{old_name} / #{new_name}", flush=True)
        await self.close()
